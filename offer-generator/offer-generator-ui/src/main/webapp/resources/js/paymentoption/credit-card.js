	...
	function formatPayDate(object){
  	 ...
  	 function formValidation() {
		  ...
		  var validation_repeat = [];
			...
			},
			{
				"id" : "cardtype",
				"validate" : true,
				"message" : cardtypeerrormsg,
				"class" : "form-cardtype"
			},
			{
				"id" : "cardmonth",
				"validate" : true,
				"message" : cardmontherrormsg,
				"class" : "form-cardmonth"
			},
			{
				"id" : "cardnumber",
				"validate" : true,
				"message" : cardnumbererrormsg,
				"class" : "form-cardnumber"
			},
			{
				"id" : "cardyear",
				"validate" : true,
				"message" : cardyearerrormsg,
				"class" : "form-cardyear"
			},
			{
				"id" : "cvv",
			...
			$.each(obj_rep, function(key, value) {
				...
				if (obj_rep[key].validate) {
                    ...
                    if(obj_rep[key].id == "payment-date"){
                	 ...
                	 if((getcombotext.length < 10 || ((getcombotext.substring(0, 2))*1) > 12) || ((getcombotext.substring(3, 5))*1) > 31){
                  	   ...
                  	   if(today > payDate){
                  		 ...
                  		 getfielddata='';
                  	   }else if($('#paymentfrequency option:selected').text()=="One Time Payment" && payDate > todayOneTimePay){
                   		...
                   		if(getcombotext != ''){
	                    	 ...
	                    	 if((getcombotext.length < 10 || ((getcombotext.substring(0, 2))*1) > 12) || ((getcombotext.substring(3, 5))*1) > 31){
	                    	   ...
	                    	   if(today > perDate){
	                    		 ...
	                    		 obj_rep[key].message = closedategreaterstartdateerrormsg;
	                    		 getfielddata='';
	                    	   }else{
					   	         getfielddata=getcombotext;
	                    	   }
                    ...
                    if(obj_rep[key].id == "cardnumber"){
                      	 getcombotext=$('#cardnumber').val();
   						 getfielddata=getcombotext;

   					 }
                    if(obj_rep[key].id == "cardtype"){
                   	 getcombotext=$('#cardtype').val();
						 getfielddata=getcombotext;

					 }
                    if(obj_rep[key].id == "cardmonth"){
                    	getcombotext=$('#cardmonth').val();
						 getfielddata=getcombotext;

					 }
                    if(obj_rep[key].id == "cardyear"){
                    	 getcombotext=$('#cardyear').val();
                        	if($('#cardyear').val()!=''){
                        		if(getcombotext.length<4){
                        			obj_rep[key].message=card4digityearerrormsg;
		                    		 getfielddata='';
                        		}else{
                        			 getfielddata=getcombotext;
                        		}
                    ...
                    if(obj_rep[key].id == "cvv"){
                   	 getcombotext=$('#cvv').val();
						 getfielddata=getcombotext;
                    ...
                    if(obj_rep[key].id == "country"){
                    	 getcombotext=$('#country').val();
						 getfielddata=getcombotext;
  	 ...
  	 **/
  	 ...
  	 $(document).ready(function() {
  		...
  		setLocaleAutoLookUp($("#city"), $("#city"), $("#state"), $("#country"), $("#zipcode"), "CITY");
		setLocaleAutoLookUp($("#state"), null, $("#state"), $("#country"), null, "STATE");
		setLocaleAutoLookUp($("#country"), null, null, $("#country"), null, "COUNTRY");
		setLocaleAutoLookUp($("#zipcode"), $("#city"), $("#state"), $("#country"), $("#zipcode"), "ZIPCODE");
 ...
 function setPaymentMethod(data){
  	 ...
  	 function accountdetails(accdetail){
  		...
  		function getData(){
  		/*	$("#validation_message_div span").remove();
	    	$("#validation_message_div br").remove();*/
	    	var temp;
			var status=true;
  		...
  		function populatedialog(){
			 ...
			 $('#dia_recurring_amount').text($('#recurring_amount').val());
  			 $('#dia_cardtype').text($("#cardtype option:selected").text());
  			 $('#dia_cardnumber').text($('#cardnumber').val());
  			 $('#dia_cardmonth').val($('#cardmonth').val());
  			 $('#dia_cardmonth').text($('#cardmonth').val()+"/"+$('#cardyear').val());
  			 $('#dia_cardyear').val($('#cardyear').val());
  			 $('#dia_cvv').text($('#cvv').val());
  			 $('#dia_name').text($('#name').val());
  			var billAddress=$('#address1').val()+", "+$('#address2').val()+", "+$('#country').val()+", "+$('#state').val()+", "+$('#zipcode').val();
  		...
  		**/
			...
			try{
					  ...
					  if(statusone == "S" || statusrec == "S"){
						  if(statusone == "S" && statusrec == "S"){
							  ...
							  $("#success_notify").append("<div><div class='submitted-txt'>"+oneTimePaySuccess+"</div></div><div><div class='submitted-txt'>"+payPlanSuccess+"</div></div><p>"+oneTimePaySuccessID+""+payResource.mpspostingid+"</p><p>"+payPlanSuccessID+""+payResource.mpspostingidrec+"</p><div class='clear-fix'></div><div class='cheque-img'><button onClick=loadPaymentReceipt("+ payResource.payId +") id='pdfId' class='btn btn-primary'>"+payReceipt+"</button></div>");

						  }else{
							  if(statusone == "S"){
								  $("#success_notify").empty();
								  $("#success_notify").append("<div><div class='submitted-txt'>"+oneTimePaySuccess+"</div></div><p>"+oneTimePaySuccessID+""+payResource.mpspostingid+"</p><div class='clear-fix'></div><div class='cheque-img'><button onClick=loadPaymentReceipt("+ payResource.payId +") id='pdfId' class='btn btn-primary'>"+payReceipt+"</button></div>");
							  }else if(statusone == "F"){
								  $("#success_notify").empty();
  		...
  		function checkpaymentsdetail(){
			 ...
			 var cardNumber=$('#dia_cardnumber').text().replace(/\-/g,"");
  			 ...
  			 formData = {
	            ...
	            "accountnumber":btoa($('#dia_accountnumber').text()),
  			    ...
  			    "firstpaymentdate":$('#dia_payment-date').text(),
  				"paytype":$('#cardtype').val(),
                "cardnumber":btoa(cardNumber),
  			    "expmonth":btoa($('#dia_cardmonth').val()),
  				"expyear":btoa($('#dia_cardyear').val()),
  				"securitycode":btoa($('#dia_cvv').text()),
  				"nameofcard":btoa($('#name').val()),
  				"address1":btoa($('#address1').val()),
				"address2":btoa($('#address2').val()),
  				//"city":$('#city').text(),
  				"cityid":btoa($('#city').val()),
  				//"state":$('#state').children(":selected").text(),
  				"stateid":btoa($('#state').val()),
  				//"country":$('#country').children(":selected").text(),
				"countryid":btoa($('#country').val()),
